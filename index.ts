...
const dateToTimeString = function(date: Date): string {
    return `${date.getHours().toString().padStart(2, '0')}:${date.getMinutes().toString().padStart(2, '0')}`
};

const dateToDateString = function(date: Date): string {
    return `${date.getFullYear().toString().padStart(4, '0')}/${(date.getMonth() + 1).toString().padStart(2, '0')}/${date.getDate().toString().padStart(2, '0')}`;
}

const dateToDateTimeString = function(date: Date): string {
...
const insertEvent = async(auth: any, databaseId: string, taskPageId: string, taskUrl: string, databaseName: string, taskProjectNamesString: string, taskTitle: string, taskStatus: string, taskStartDate: Date | null, taskEndDate: Date | null, taskPersonNamesString: string) => {
	...
	const sameEvents = searchResult.data.items.filter((event: any) => {
...
const refresh = async() => {
	...
	for(const databasePage of databasePages) {
		...
		const callout = databaseBlocks.reverse().find((block: NotionObject) => {
		...
		const toggle = callOutBlocks.find((block: NotionObject) => {
		...
		const DoNotErasePage = toggleBlocks.find((block: NotionObject) => {
		...
		const TasksDatabase = DoNotErasePageBlocks.find((block: NotionObject) => {
		...
		let taskPages = [];
		while (hasMore) {
			const taskPagesResponse: NotionObject = await notion.databases.query({
		...
		for(const taskPage of taskPages) {
			...
			const taskProjectNamePromises = taskProjectPages.map(async (page: NotionObject) => {
			...
			const taskPersonNamePromises = taskPersonPages.map(async (page: NotionObject) => {
				const pageResponse = await notion.pages.retrieve({ page_id: page.id });
